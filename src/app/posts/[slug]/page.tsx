...
const mainGetPostEffect = (slug: string) =>
  Effect.provide(
    Effect.gen(function* (_) {
      return yield* _(
        fetchGeneric(PostSchema, `http://localhost:3000/api/posts/${slug}`, {
...
const mainGetPostsEffect = Effect.provide(
  Effect.gen(function* (_) {
    return yield* _(
      fetchGeneric(PostsSchema, `http://localhost:3000/api/posts`, {
        next: { revalidate: 3600 },
      }),
    );
  }),
...
export default async function PostPage({
  ...
  const res = await Effect.runPromise(
    mainGetPostEffect(parsedSearchParams.slug),
  );
